---
title: Timetable
layout: coursebook
---

# Timetable

Content is arranged by blocks (single week of teaching content).

There are 3 summative assessments (Assessment 1-2 + Portfolio), plus a formative (non-assessed) assessment and portfolio.
### Semester 1

* [00 About](coursebook/00.md)
* Week 1: Block [01 Introduction to Data Science](coursebook/01.md)
    * [Assessment 0]({{ site.data.assessment0.url }}) *Set*
    * [Portfolio 0]({{ site.data.individualassessment0.url }}) *Set*
* Week 2: Block [02 Modern Regression and Cross Validation](coursebook/02.md)
* Week 3: Block [03 Loss Minimisation for Decisions, Boosting, Forests](coursebook/03.md)
    * [Assessment 0]({{ site.data.assessment0.url }}) *Due* (Wednesday noon)
    * [Assessment 1]({{ site.data.assessment1.url }}) *Set*
* Week 4: Block [04 Unsupervised learning: Latent Structures](coursebook/04.md)
    * [Portfolio 0]({{ site.data.individualassessment0.url }}) *Due* (Wednesday noon)
    * [Portfolio]({{ site.data.individualassessment1.url }}) *Set*
* Week 5: **Lecture contents from** Block [05 Unsupervised learning: Outliers and Missingness](coursebook/05.md)
 [06 Decision Trees and Random Forests](coursebook/06.md)
* Week 6: Consolidation Week. **No lectures**.
* Week 8: Block [06 Perceptrons and Neural Networks](coursebook/07.md)
    * [Assessment 1]({{ site.data.assessment1.url }}) *Due* (Wednesday noon)
    * [Assessment 2]({{ site.data.assessment2.url }}) *Set*
* Week 9: Block [07 Topic Models and Bayesian Methods](coursebook/08.md)
* Week 10: Block [08 Algorithms for Data Science](coursebook/09.md)
* Week 11: Block [09 Parallel Algorithms](coursebook/10.md)
* Week 11: Block [10 Ethics and Privacy](coursebook/11.md)
    * [Assessment 2]({{ site.data.assessment2.url }}) *Due* (Wednesday noon)
* Week 12: Assessment preparation week
    * [Portfolio]({{ site.data.individualassessment1.url }}) *Due* (**Tuesday 4pm**)
